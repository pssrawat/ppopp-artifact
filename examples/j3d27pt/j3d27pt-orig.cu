...
__global__ void j3d27pt (double * __restrict__ t_in, double * __restrict__ t_out, int N) {
	...
	int j0 = (int)(blockIdx.y)*(int)(blockDim.y) + 1;
	int j = max(j0,1) + (int)(threadIdx.y);
...
extern "C" void host_code (double *h_in, double *h_out, int N) {
	...
	dim3 gridconfig (ceil(N-2, blockconfig.x), ceil(N-2, blockconfig.y), ceil(N-2, blockconfig.z));
