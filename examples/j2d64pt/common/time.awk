...
awk -v otime=$time 'BEGIN {print "Original GFlops = " (8192*8192*73/10^6/otime)}'

time=`grep -E 'float|double' nvcc-unroll-results | awk 'BEGIN {time = 0.0} {time += $2} END {print time}'`
awk -v utime=$time 'BEGIN {print "Unrolled GFlops = " (8192*8192*73/10^6/utime)}'
...
awk -v min=$min3 -v etime=$timee 'BEGIN {print "Reordered GFlops = " (8192*8192*73/10^6/(min<etime?min:etime))}'
