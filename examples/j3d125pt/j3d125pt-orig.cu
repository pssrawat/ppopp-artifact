...
__global__ void j3d125pt (double * __restrict__ t_in, double * __restrict__ t_out, int N) {
	...
	int j0 = (int)(blockIdx.y)*(int)(blockDim.y) + 2;
	int j = max(j0,2) + (int)(threadIdx.y);
	...
	if (i<=N-3 & j<=N-3 && k<=N-3) {
		out[k][j][i] =
			0.75 * (in[k-2][j-2][i-2] + in[k-2][j-2][i+2] + in[k-2][j+2][i-2] + in[k-2][j+2][i+2] + in[k-1][j-1][i-1] + in[k-1][j-1][i+1] + in[k-1][j+1][i-1] + in[k-1][j+1][i+1] + 
			in[k][j-1][i] + in[k][j][i-1] +  in[k][j][i+1] + in[k][j+1][i] + 
			in[k+1][j-1][i-1] + in[k+1][j-1][i+1] + in[k+1][j+1][i-1] + in[k+1][j+1][i+1]) + 
			0.76 * (in[k-2][j-2][i-2] + in[k-2][j-2][i+2] + in[k-2][j+2][i-2] + in[k-2][j+2][i+2]) + 

			1.132 * (in[k-2][j-2][i-1] + in[k-2][j-2][i+1] + in[k-2][j-1][i-2] + in[k-2][j-1][i+2] + in[k-2][j][i] + in[k-2][j+1][i-2] + in[k-2][j+1][i+2] + in[k-2][j+2][i-1] +  in[k-2][j+2][i+1] +
			in[k-1][j-2][i-2] + in[k-1][j-2][i+2] + in[k-1][j+2][i-2] + in[k-1][j+2][i+2] + 
			in[k][j-2][i] + in[k][j][i-2] + in[k][j][i+2] + in[k][j+2][i] +  
			in[k+1][j-2][i-2] + in[k+1][j-2][i+2] + in[k+1][j+2][i-2] + in[k+1][j+2][i+2] + 
			in[k-2][j-2][i-1] + in[k-2][j-2][i+1] + in[k-2][j-1][i-2] + in[k-2][j-1][i+2] + in[k-2][j][i] + in[k-2][j+1][i-2] + in[k-2][j+1][i+2] + in[k-2][j+2][i-1] +  in[k-2][j+2][i+1]) +
			...
			in[k][j-2][i-1] + in[k][j-2][i+1] + in[k][j-1][i-2] + in[k][j-1][i+2] + in[k][j][i] + in[k][j+1][i-2] + in[k][j+1][i+2] + in[k][j+2][i-1] +  in[k][j+2][i+1] +
			in[k+1][j-2][i] + in[k+1][j][i-2] + in[k+1][j][i+2] + in[k+1][j+2][i] +  
			in[k-2][j-1][i] + in[k-2][j][i-1] +  in[k-2][j][i+1] + in[k-2][j+1][i]) + 

			0.331 * (in[k-2][j-1][i-1] + in[k-2][j-1][i+1] + in[k-2][j+1][i-1] + in[k-2][j+1][i+1] + 
			in[k-1][j-2][i-1] + in[k-1][j-2][i+1] + in[k-1][j-1][i-2] + in[k-1][j-1][i+2] + in[k-1][j][i] + in[k-1][j+1][i-2] + in[k-1][j+1][i+2] + in[k-1][j+2][i-1] +  in[k-1][j+2][i+1] +
			in[k][j-1][i-1] + in[k][j-1][i+1] + in[k][j+1][i-1] + in[k][j+1][i+1] + 
			in[k+1][j-2][i-1] + in[k+1][j-2][i+1] + in[k+1][j-1][i-2] + in[k+1][j-1][i+2] + in[k+1][j][i] + in[k+1][j+1][i-2] + in[k+1][j+1][i+2] + in[k+1][j+2][i-1] +  in[k+1][j+2][i+1]) +
			0.332*(in[k-2][j-1][i-1] + in[k-2][j-1][i+1] + in[k-2][j+1][i-1] + in[k-2][j+1][i+1]); 
	}
}
...
extern "C" void host_code (double *h_in, double *h_out, int N) {
	...
	dim3 gridconfig (ceil(N-4, blockconfig.x), ceil(N-4, blockconfig.y), ceil(N-4, blockconfig.z));
